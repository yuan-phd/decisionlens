...
#   F1-macro : ~0.55
#   F1-Terminated (minority class=0) : ~0.17 at threshold=0.1, spw=5.0
#
...
class EnrollmentForecaster:
    ...
    def fit(self, df: pd.DataFrame) -> "EnrollmentForecaster":
        ...
        log.info("Regressor fitted on %d samples.", len(reg_df))
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            DataFrame (one row per input row) with columns:
                ...
                - pred_label         : 1 if p_completed ≥ 0.5 else 0
                - pred_duration_days : predicted enrollment_duration_days (≥ 0)
        ...
        result["pred_label"]         = (result["p_completed"] >= 0.5).astype(int)
        result["pred_duration_days"] = self._reg_pipeline.predict(X).clip(min=0)
